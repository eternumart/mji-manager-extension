export const launchApp = (currentFio: string, login: string, loginIsPossible: boolean, launchStatus: boolean, appDataString: any) => {
	...
	const representativesInputs: any = {
		empty: true,
	};
	const allRatesPercentsInputs: any = {};

	// Предотвращение двойного старта
	if (!localStorage.getItem("status")) {
		setToStorage(false, launchStatus, null, null);
	} else {
		const storageData = JSON.parse(localStorage.getItem("status") as string);
		if (storageData.init) {
			return;
		} else {
			if (loginIsPossible) {
    ...
    appVariables.html = (document.querySelector("#formCanvas") as HTMLIFrameElement).contentWindow!.document.querySelector("html") || document.querySelector("html");
	...
	function createPopup(currentPage: any) {
	...
	function setToStorage(layout: any, init: any, authorized: any, uid: any) {
		let status: any;
		if (localStorage.getItem("status")) {
	...
	function minimizeAppByEscape(evt: any) {
		if (evt.key === "Escape") {
			minimizeApp();
		}
	}
	...
	function setInitialDate(tag: any) {
		const date = new Date();
	...
	function changeTab(clickedTab: any) {
		...
		appVariables.tabsContent.forEach((content: any) => {
	...
	function startDraggingDiv(event: any) {
	...
	function buttonError(button: any, currentPage: any, needPage: any, stdValue: any) {
	...
	function downloadPhotos(evt: any) {
		evt.preventDefault();
	...
	function clickGenerator(ocenkaInput: any, value: any, load: any) {
		if (load && value === "-") {
			return;
		}
		try {
	...
	function setRepresentatives() {
		...
		Object.keys(representativesInputs).forEach((key: any) => {
	...
	function splitBySentences(text: any) {
		if (text === "") {
			return [""];
		}
		const regex = /[.!?]\s+/g;
	...
	function openFakeSelect(selectList: any) {
	...
	function closeFakeSelect(selectList: any) {
